...
IMPLEMENT_SERIAL(AND_Gate, CObject, 1)

AND_Gate::AND_Gate(const CPoint& iPos) : Component(iPos, 0, 4)
{
	setPos(iPos);
...
AND_Gate::AND_Gate() : Component(CPoint(0, 0), 0, 4)
{
...
void AND_Gate::Serialize(CArchive& ar)
{
...
void AND_Gate::computeResult()
{
	if (mCons[0]->getState() == 1 && mCons[1]->getState() == 1)
		mCons[2]->setState(true);
	else
		mCons[2]->setState(false);
}

Node* AND_Gate::getNode(const int iNodeIndex) const
{
	return mCons[iNodeIndex];
}

void AND_Gate::Draw(CDC& dc)
{
	CBrush brush;
	...
	dc.MoveTo(pos.x + 25, pos.y - 25);
	dc.LineTo(pos.x + 25, pos.y - 50);
	dc.MoveTo(pos.x, pos.y + 10);
	dc.LineTo(pos.x, pos.y + 50);

	// draw half circle
	dc.Chord(pos.x + 32, pos.y + 25, pos.x - 32, pos.y - 75, pos.x - 32, pos.y - 25, pos.x + 32, pos.y - 25);
...
void AND_Gate::Move(const CPoint iPos)
{
	setPos(iPos);
...
bool AND_Gate::IsInside(const CPoint mousePos) const
{
	CPoint pos = getPos();
